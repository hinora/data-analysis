...
function buildSystemPrompt(req: { datasets: DatasetInfo[] }): string {
  ...
  if (visualization.length > 0) {
    parts.push(
      ...
      "- Be PROACTIVE: whenever your analysis produces numerical results that would be clearer as a chart, use `generateChartSpec` to create one — even if the user did not explicitly ask for a visualization.",
      "- First use aggregation/retrieval tools to get the data, then pass the aggregated results to `generateChartSpec`.",
      ...
      "- You can call `generateChartSpec` multiple times in one response to produce multiple charts.",
      "- Each chart is numbered starting from 0, in the order you call `generateChartSpec`.",
      "- In your final text response, place the placeholder `[chart:N]` (e.g. `[chart:0]`, `[chart:1]`) at the exact position where you want the chart to appear.",
      "- Example: 'Here is the revenue breakdown:\\n\\n[chart:0]\\n\\nAnd the trend over time:\\n\\n[chart:1]'",
      "- If you do not include `[chart:N]` placeholders, charts will be appended at the end of your response.",
      "",
